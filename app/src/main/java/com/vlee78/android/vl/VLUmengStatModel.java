...
public abstract class VLUmengStatModel extends VLModel implements VLActivityManager.VLActivityListener {
    ...
    protected void setAppKey(String appKey, String channel) {
        VLDebug.Assert(appKey != null && appKey.length() > 0);
        MobclickAgent.UMAnalyticsConfig config = new MobclickAgent.UMAnalyticsConfig(getConcretApplication(), appKey, channel);
        if (mEnabled) MobclickAgent.startWithConfigure(config);
    }
